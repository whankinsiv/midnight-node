...
//! Verification of Reserve Authorization Scripts
//!
//! This module verifies that the reserve validator contract is linked to the expected
...
/// Reserve addresses including compiled code and two-stage policy ID
#[derive(Debug, Clone, Deserialize)]
pub struct ReserveAddressesWithCode {
	pub reserve_validator_address: String,
	#[serde(deserialize_with = "hex_to_policy_id")]
	pub reserve_validator_policy_id: PolicyId,
	pub reserve_validator_compiled_code: String,
	#[serde(deserialize_with = "hex_to_policy_id")]
	pub reserve_validator_two_stage_policy_id: PolicyId,
}

/// Result of all verification checks for Reserve
#[derive(Debug)]
pub struct ReserveVerificationResult {
...
impl ReserveVerificationResult {
	...
	pub fn print_summary(&self) {
		println!("\n=== Reserve Auth Script Verification ===\n");

		print_check("Reserve Policy Hash", &self.policy_hash_check);
		print_check("Reserve Two-Stage Embedded", &self.two_stage_embedded_check);
...
/// Load reserve addresses with compiled code from JSON file
fn load_reserve_addresses(path: &Path) -> Result<ReserveAddressesWithCode, VerifyAuthScriptError> {
	let content = std::fs::read_to_string(path)?;
	let addresses: ReserveAddressesWithCode = serde_json::from_str(&content)?;
	Ok(addresses)
}

/// Main verification function for Reserve
pub async fn verify_reserve_auth_script(
	reserve_addresses_path: &Path,
	authorization_addresses_path: Option<&Path>,
	pool: &PgPool,
	cardano_tip: &McBlockHash,
) -> Result<ReserveVerificationResult, VerifyAuthScriptError> {
	log::info!("Loading reserve addresses from {}", reserve_addresses_path.display());
	let addresses = load_reserve_addresses(reserve_addresses_path)?;

	// Check 1: Reserve policy hash
	let policy_hash_check = verify_policy_hash(
		"Reserve",
		&addresses.reserve_validator_compiled_code,
		&addresses.reserve_validator_policy_id,
	);

	// Check 2: Reserve two_stage_policy_id embedded
	let two_stage_embedded_check = verify_two_stage_embedded(
		"Reserve",
		&addresses.reserve_validator_compiled_code,
		&addresses.reserve_validator_two_stage_policy_id,
	);

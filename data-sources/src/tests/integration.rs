use crate::tests::{
	...
	grpc_db_sync::{
		...
		federated_authority::test_grpc_federated_authority_against_db_sync, load_params_from_grpc,
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
	...
	let params = load_params_from_grpc(&config)
		.await
		.map_err(|e| format!("params failed: {e}"))?;

	test_grpc_cnight_observation_against_db_sync(&config, &params)
		.await
		.map_err(|e| format!("CNight observation test failed: {e}"))?;
	println!("CNight observation passed");

	test_grpc_authority_selection_against_db_sync(&config, &params)
		.await
		.map_err(|e| format!("authority selection test failed: {e}"))?;
	println!("Authority selection passed");

	test_grpc_federated_authority_against_db_sync(&config, &params)
		.await
		.map_err(|e| format!("federated authority test failed: {e}"))?;
	println!("Federated authority passed");

	test_grpc_mc_hash_grpc_against_db_sync(&config, &params)
		.await
